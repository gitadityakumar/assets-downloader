//! Cupcake provider — Jonas Nilsson Lee's free CC0 stock photography.
//! Source: https://cupcake.nilssonlee.se/
//! Download: Direct master image from ImageKit CDN without size transforms.
...
pub const id = "cupcake";
pub const name = "Cupcake";
...
fn parseSearch(allocator: Allocator, html: []const u8, query: []const u8, limit: u32) ![]Asset {
    ...
    const match_all = std.mem.eql(u8, trimmed_query, "*") or
        ...
        std.ascii.eqlIgnoreCase(trimmed_query, "cupcake") or
    ...
    while (assets.items.len < limit) {
        ...
        if (std.mem.indexOf(u8, src_val, "imagekit.io") == null) continue;
        ...
        // Master URL: strip query parameters
        const q_pos = std.mem.indexOfScalar(u8, src_val, '?');
        const master_raw = if (q_pos) |qp| src_val[0..qp] else src_val;

        // Determine ID from file basename
        const last_slash = std.mem.lastIndexOfScalar(u8, master_raw, '/') orelse continue;
        var base_name = master_raw[last_slash + 1 ..];
        if (std.mem.lastIndexOfScalar(u8, base_name, '.')) |dot_idx| {
            base_name = base_name[0..dot_idx];
        }

        if (seen.contains(base_name)) continue;
        try seen.put(allocator, try allocator.dupe(u8, base_name), {});

        const pid = try allocator.dupe(u8, base_name);
        errdefer allocator.free(pid);

        const desc = if (alt_clean.len > 0) alt_clean else base_name;
...
pub fn search(
    ...
    const search_url = "https://cupcake.nilssonlee.se/";
    var resp = try http_client.get(client, allocator, search_url, .{
        .accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
        .referer = "https://cupcake.nilssonlee.se/",
        .max_redirects = 5,
    });
    defer resp.deinit();
