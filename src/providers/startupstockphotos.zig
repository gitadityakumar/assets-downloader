//! Startup Stock Photos provider — free stock photography for startups, bloggers, and tech.
...
pub const id = "startupstockphotos";
pub const name = "Startup Stock Photos";

fn fullSizeUrl(allocator: Allocator, thumb: []const u8) ![]u8 {
    // .../ssp-work-tech-0015-500x330.jpg → .../ssp-work-tech-0015.jpg
...
fn idFromUrl(allocator: Allocator, page_url: []const u8) ![]u8 {
    // https://startupstockphotos.com/photos/workspace-desk-top/ → workspace-desk-top
    var u = page_url;
...
fn cleanTitle(s: []const u8) []const u8 {
    ...
    if (std.mem.endsWith(u8, t, " Free Stock Picture")) {
        t = t[0 .. t.len - 19];
    } else if (std.mem.endsWith(u8, t, " Free Stock Photo")) {
        t = t[0 .. t.len - 17];
    }
...
fn parseSearch(allocator: Allocator, html: []const u8, limit: u32) ![]Asset {
    ...
    const marker = "href=\"https://startupstockphotos.com/photos/";
    var pos: usize = 0;
    while (assets.items.len < limit) {
        const found = std.mem.indexOfPos(u8, html, pos, marker) orelse break;
        pos = found + marker.len;
        const href_start = found + 6; // start at https://...
        const href_end = std.mem.indexOfScalarPos(u8, html, href_start, '"') orelse continue;
        const href = html[href_start..href_end];

        const window_end = @min(html.len, found + 2500);
        ...
        const title: []const u8 = blk: {
            ...
            if (std.mem.indexOf(u8, win, alt_key)) |ai| {
                ...
                const te = std.mem.indexOfScalarPos(u8, win, ts, '"') orelse break :blk "";
                break :blk cleanTitle(win[ts..te]);
            }
            break :blk "";
        };
        ...
        try meta_aw.writer.print("{{\"source\":\"startupstockphotos\",\"page\":\"{s}\"}}", .{href});
        ...
        try assets.append(allocator, .{
            ...
            .author = try allocator.dupe(u8, "Startup Stock Photos"),
            .width = null,
            .height = null,
            .metadata_json = meta,
        });
...
pub fn search(
    ...
    const search_url = try std.fmt.allocPrint(allocator, "https://startupstockphotos.com/?s={s}", .{enc});
    defer allocator.free(search_url);

    var resp = try http_client.get(client, allocator, search_url, .{
        .accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
        .referer = "https://startupstockphotos.com/",
        .max_redirects = 5,
    });
    defer resp.deinit();
