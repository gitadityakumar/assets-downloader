//! ISO Republic provider — WordPress search HTML.
//! Search: GET /?s={query}
...
pub const id = "isorepublic";
pub const name = "ISO Republic";

fn fullSizeUrl(allocator: Allocator, thumb: []const u8) ![]u8 {
    // .../name-450x300.jpg → .../name.jpg
...
fn idFromUrl(allocator: Allocator, page_url: []const u8) ![]u8 {
    // https://isorepublic.com/photo/woman-fall-nature/ → woman-fall-nature
    var u = page_url;
...
fn parseSearch(allocator: Allocator, html: []const u8, limit: u32) ![]Asset {
    ...
    const marker = "class=\"photo-grid-item";
    var pos: usize = 0;
    while (assets.items.len < limit) {
        ...
        if (std.mem.indexOf(u8, href, "/photo/") == null) continue;

        const title_key = "title=\"";
        const title_i = std.mem.indexOf(u8, win, title_key);
        ...
        const thumb = win[src_start..src_end];
        if (std.mem.indexOf(u8, thumb, "wp-content/uploads/") == null) continue;
        if (std.mem.indexOf(u8, thumb, "shutterstock") != null) continue;
        ...
        const full = try fullSizeUrl(allocator, thumb);
        ...
        try meta_aw.writer.print("{{\"source\":\"isorepublic\",\"page\":\"{s}\"}}", .{href});
...
pub fn search(
    ...
    const search_url = try std.fmt.allocPrint(allocator, "https://isorepublic.com/?s={s}", .{enc});
